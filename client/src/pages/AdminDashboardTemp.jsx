      ...
      <aside className="w-80 bg-gray-900 text-white flex flex-col fixed h-full z-20 shadow-[20px_0_60px_-15px_rgba(0,0,0,0.1)]">
        <div className="p-10 border-b border-white/5">
          <div className="flex items-center space-x-4 mb-4 group cursor-pointer">
            ...
            <div>
              ...
              <p className="text-[9px] font-bold text-emerald-500 tracking-[0.2em] mt-1.5 uppercase">Pharmacy Management OS</p>
            </div>
          </div>
        ...
        <div className="p-10">
          <div className="bg-emerald-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
            ...
            <p className="text-[9px] font-bold text-emerald-500 tracking-[0.2em] mt-1.5 uppercase">Pharmacy Management OS</p>
            <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">Core Frequency</p>
        ...
        {activeTab === "overview" && stats && (
          <div className="animate-fade-in pb-20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-20">
              {[
                { label: "Gross Manifest Valuation", value: `${stats.totalRevenue?.toLocaleString() || 0}`, unit: "ETB", icon: FaDollarSign, color: "emerald", trend: "+12.5%" },
                { label: "Relayed Protocols", value: stats.totalOrders || 0, unit: "UNITS", icon: FaShoppingBag, color: "blue", trend: "+8.2%" },
                ...
                <div className="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl relative z-10 max-h-[90vh] overflow-y-auto">
                  <div className="p-12">
                    ...
                    <form onSubmit={handleMedicineSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      ...
                      <div className="md:col-span-2">
                        <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Pharmacy Description</label>
                      ...
                      <td className="px-10 py-6">
                        <div className="flex items-center space-x-4">
                          <div className="flex-1 bg-slate-100 h-2 rounded-full w-24 overflow-hidden border border-slate-50 shadow-inner">
                            <div className={`h-full ${item.stock > 20 ? 'bg-emerald-500' : item.stock > 10 ? 'bg-amber-500' : 'bg-rose-500'} transition-all`} style={{ width: `${Math.min(100, (item.stock / 50) * 100)}%` }}></div>
                          </div>
        ...
        {activeTab === "orders" && (
          <div className="animate-fade-in">
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
              {orders.map((order) => (
                <div key={order._id} className="bg-white rounded-[3rem] shadow-xl shadow-gray-200/40 border border-gray-50 p-8 hover:shadow-2xl transition-all duration-500">
                  <div className="flex justify-between items-start mb-8">
                    <div>
                      <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2">Order Protocol</h4>
                      <p className="text-2xl font-black text-gray-900 tracking-tighter">#{order._id?.slice(-8).toUpperCase()}</p>
                    </div>
                    <div className={`px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest border ${getStatusStyle(order.status)}`}>
                      {order.status}
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    <div>
                      <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4">Customer Data</h4>
                      <div className="bg-gray-50 p-6 rounded-2xl">
                        <p className="font-black text-gray-900">{order.user?.name || 'Unknown'}</p>
                        <p className="text-[10px] text-gray-400 uppercase tracking-wider mt-2">{order.deliveryAddress?.street}, {order.deliveryAddress?.city}</p>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      {order.items?.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-center p-4 bg-gray-50 rounded-2xl border border-gray-100 mb-3">
                          <div className="flex items-center space-x-4">
                      ...
                      <div className="flex justify-between items-center px-6 pt-6 border-t border-gray-100">
                    ...
                    <div className="flex flex-col space-y-3">
                      {!order.payment?.confirmed && (
                        <button onClick={() => handleConfirmPayment(order._id)} className="w-full px-6 py-4 bg-emerald-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-700 transition-all">
                          Confirm Payment
                        </button>
                      )}
                      <div className="grid grid-cols-2 gap-3">
                        {["pending", "processing", "shipped", "delivered"].map((status) => (
                          <button
                            ...
                            className={`px-4 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${order.status === status
